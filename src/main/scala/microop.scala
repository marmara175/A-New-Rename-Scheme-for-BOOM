...
class MicroOp(implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   val vdst             = UInt(width = TPREG_SZ)
   val vop1             = UInt(width = TPREG_SZ)
   val vop2             = UInt(width = TPREG_SZ)
   val vop3             = UInt(width = TPREG_SZ)
   val stale_vdst       = UInt(width = TPREG_SZ)

   val pdst             = UInt(width = TPREG_SZ)
   val pop1             = UInt(width = TPREG_SZ)
   val pop2             = UInt(width = TPREG_SZ)
   val pop3             = UInt(width = TPREG_SZ)
