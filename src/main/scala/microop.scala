...
class MicroOp(implicit p: Parameters) extends BoomBundle()(p)
{
   ...
   val vdst             = UInt(width = VPREG_SZ)
   val vop1             = UInt(width = VPREG_SZ)
   val vop2             = UInt(width = VPREG_SZ)
   val vop3             = UInt(width = VPREG_SZ)
   val stale_vdst       = UInt(width = VPREG_SZ)

   val pdst             = UInt(width = IPPREG_SZ)
   val pop1             = UInt(width = IPPREG_SZ)
   val pop2             = UInt(width = IPPREG_SZ)
   val pop3             = UInt(width = IPPREG_SZ)

   val dst_mask         = UInt(width = numIntPhysRegsParts)
   val rs1_mask         = UInt(width = numIntPhysRegsParts)
